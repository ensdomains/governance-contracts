...
module.exports = async ({getNamedAccounts, deployments}) => {
  ...
  const timelockedSupply = ethers.BigNumber.from(10).pow(18).mul(process.env.TIMELOCKED_SUPPLY);
  ...
  await ensToken.transfer(timelockController.address, ethers.BigNumber.from(10).pow(18).mul(process.env.FREE_SUPPLY));
};
