...
describe('StringUtils', () => {
  ...
  it('should escape double quote correctly based JSON standard', async () => {
    expect(await stringUtils.testEscape('My ENS is, "tanrikulu.eth"')).to.equal('My ENS is, \\"tanrikulu.eth\\"');
  });

  it('should escape backslash correctly based JSON standard', async () => {
    expect(await stringUtils.testEscape('Path\\to\\file')).to.equal('Path\\\\to\\\\file');
  });
